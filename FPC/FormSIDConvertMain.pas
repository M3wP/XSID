...
uses
	...
	Dialogs, StdCtrls, VirtualTrees, SIDConvTypes, ExtCtrls, MaskEdit;
...
type
    ...
    TSIDConvertMainForm = class(TForm)
		...
		procedure Memo2DragDrop(Sender, Source: TObject; X, Y: Integer);
...
uses
	Types, LazFileUtils, FileUtil, Process, FormSIDConvertConfig, FormSIDConvProgress,
    SIDPlay;
...
procedure TSIDConvertMainForm.Button2Click(Sender: TObject);
    ...
    try
        ...
        //proc.CommandLine:= FConfig.viceVSID + ' ' + '-tune ' + IntToStr(ss) + ' ' +
                //FProcessFiles[FFocusedData^.fileIndex];

        proc.Executable:= FConfig.viceVSID;
        proc.Parameters.Add('-tune');
        proc.Parameters.Add(IntToStr(ss));
        proc.Parameters.Add(FProcessFiles[FFocusedData^.fileIndex]);
	    ...
	    repeat
		    ...
		    for i:= 0 to data^.header.songs - 1 do
			    ...
			    if  i in data^.selected then
				    ...
				    if  n = '' then
					    n:= 'Unknown';
				    f:= IncludeTrailingPathDelimiter(FConfig.outputPath) + n;

				    n:= StripChars('?/\*&:<>', string(data^.header.name));
				    if  n = '' then
					    n:= 'Unknown';
				    f:= IncludeTrailingPathDelimiter(f) + n;

				    ForceDirectories(f);

				    f:= IncludeTrailingPathDelimiter(f) + Format('%2.2d - ', [i + 1]) +
						    LazFileUtils.ExtractFileNameOnly(FProcessFiles[data^.fileIndex]);
				    f:= ChangeFileExt(f, '.log');
...
procedure TSIDConvertMainForm.FormDropFiles(Sender: TObject;
        const FileNames: array of String);
    var
    i: Integer;
    ...
    for i:= Low(FileNames) to High(FileNames) do
        FAppendFiles.Add(FileNames[i]);
    end;

procedure TSIDConvertMainForm.Memo1DragDrop(Sender, Source: TObject; X,
		Y: Integer);
    begin
//
    end;

procedure TSIDConvertMainForm.Memo1DragOver(Sender, Source: TObject; X,
		Y: Integer; State: TDragState; var Accept: Boolean);
    begin
    Accept:= False;
    end;

procedure TSIDConvertMainForm.Memo2DragDrop(Sender, Source: TObject; X,
		Y: Integer);
    begin
//
    end;

procedure TSIDConvertMainForm.Memo2DragOver(Sender, Source: TObject; X,
		Y: Integer; State: TDragState; var Accept: Boolean);
    begin
    Accept:= False;
    end;
...
procedure TSIDConvertMainForm.ProcessAppendFiles;
	...
	procedure DoProcessDirectory(const APath: string);
		...
		if  DirectoryExists(APath) then
			begin
            sda:= FindAllFiles(APath, '*.sid', True);
            try
			    for i:= 0 to sda.Count - 1 do
				    DoProcessFile(sda[i]);
	...
	for i:= 0 to FAppendFiles.Count - 1 do
		if  Length(ExtractFileExt(FAppendFiles[i])) = 0 then
