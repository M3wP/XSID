...
uses
	...
	XSIDFiles, XSIDTypes, XSIDThread, XSIDToMIDITypes, Spin, Buttons;
...
procedure TXSIDToMIDIMainForm.Button10Click(Sender: TObject);
	...
	begin
	SaveDialog1.InitialDir:= Edit6.Text;
	...
	if  SaveDialog1.Execute then
		...
		try
			...
			f.WriteString('Project', 'XSID', Edit5.Text);
			f.WriteString('Project', 'Path', Edit6.Text);
...
procedure TXSIDToMIDIMainForm.Button11Click(Sender: TObject);
	...
	begin
	OpenDialog2.InitialDir:= Edit6.Text;
	...
	if  OpenDialog2.Execute then
		...
		try
			DoClearData;
			Edit5.Text:= f.ReadString('Project', 'XSID', '');
			Edit6.Text:= f.ReadString('Project', 'Path',
					ExtractFileDir(Application.ExeName));

			DoLoadFile(Edit5.Text);
...
//		DumpProgressForm.ProgressBar1.Position:= 1;
		...
		f:= TFileStream.Create(Format('%sins%2.2d.mid', [
				IncludeTrailingPathDelimiter(Edit6.Text), ins + 1]), fmCreate);
		try
			WriteSMFFile(smf, f);
...
procedure TXSIDToMIDIMainForm.Button2Click(Sender: TObject);
	...
	try
		...
		DoDumpFile(events, IncludeTrailingPathDelimiter(Edit6.Text) +
...
procedure TXSIDToMIDIMainForm.Button3Click(Sender: TObject);
	...
	try
		...
		DoDumpFile(events, IncludeTrailingPathDelimiter(Edit6.Text) + 'filt.log');
...
//			DumpProgressForm.ProgressBar1.Position:= i + 1;
		...
		f:= TFileStream.Create(IncludeTrailingPathDelimiter(Edit6.Text) + 'song.mid', fmCreate);
		try
			WriteSMFFile(smf, f);
...
procedure TXSIDToMIDIMainForm.DoDumpFile(const AEvents: TList; const AFileName: string);
	...
	try
        ...
        o:= TFileStream.Create(AFileName, fmCreate);
        try
            f.Position:= 0;
            o.CopyFrom(f, f.Size);
...
//		FDumpData[ASlt].XSIDConfig.Renderer:= GlobalRenderers.Items[0].GetName;
		...
		p:= IncludeTrailingPathDelimiter(Edit6.Text);
...
//	DumpProgressForm.Hide;
	...
	for slt:= 0 to FDumpSlots - 1 do
		if  Assigned(FDumpData[slt].XSIDThread) then
			...
			FDumpData[slt].XSIDThread.PausedSignal.WaitFor(INFINITE);
...
procedure TXSIDToMIDIMainForm.FormCreate(Sender: TObject);
	...
	Edit6.Text:= ExtractFileDir(Application.ExeName);
	end;
...
procedure TXSIDToMIDIMainForm.SpeedButton1Click(Sender: TObject);
	begin
...
procedure TXSIDToMIDIMainForm.SpeedButton2Click(Sender: TObject);
	begin
	SelectDirectoryDialog1.InitialDir:= Edit6.Text;

	if  SelectDirectoryDialog1.Execute then
		Edit6.Text:= SelectDirectoryDialog1.FileName;
	end;
