...
uses
	Classes, ReSIDTypes, DirectSound;

type

{ TReSIDAudioDSound }

	TReSIDAudioDSound = class(TReSIDAudioRenderer)
	protected
...
constructor TReSIDAudioDSound.Create(const ASampleRate: TReSIDSampleRate;
...
destructor TReSIDAudioDSound.Destroy;
	begin
{$IFDEF DCC}
	FBuffer:= nil;
	FDSound:= nil;
...
class function TReSIDAudioDSound.GetName: AnsiString;
	begin
	Result:= 'DirectSound Output';
	end;

class function TReSIDAudioDSound.GetWantPlatformDefault: Boolean;
	begin
//dengland If we're here then we do...
	Result:= True;
	end;

procedure TReSIDAudioDSound.Pause(var ABuffer: PArrSmallInt);
	begin
	FBuffer.Stop;
	end;

procedure TReSIDAudioDSound.Play(var ABuffer: PArrSmallInt);
	begin
	if  FBuffer.Play(0, 0, DSBPLAY_LOOPING) <> DS_OK then
		raise Exception.Create('Unable to start initial audio playback');
	end;

procedure TReSIDAudioDSound.SwapBuffers(var ABuffer: PArrSmallInt;
		const ASize: Integer);
	var
	curHalf: Integer;
	expectPos: Cardinal;
...
procedure TReSIDAudioDSound.TransferBuffer(const ABuffer: PArrSmallInt;
		var ASize: Integer);
	var
	curHalf: Integer;
	expectPos: Cardinal;
...
initialization
	RegisterRenderer(TReSIDAudioDSound);
