...
uses
	{Zip, ZLib, }IOUtils, ULZBinTree, URangeEncoder, ULZMAEncoder;
...
//	z: TZipFile;
	...
	begin
	if  FileExists(ButtonedEdit1.Text) then
		begin
		if  CheckBox6.Checked then
			s:= TPath.ChangeExtension(ButtonedEdit1.Text, 'zxsid')
		else
			s:= TPath.ChangeExtension(ButtonedEdit1.Text, 'xsid');
...
//				try
//					z.Open(s, zmWrite);
//					z.Add(m, TPath.ChangeExtension(ButtonedEdit1.Text, 'xsid'),
//							zcDeflate);
//					z.Close;
//
//					finally
...
//	c: TCompressionStream;
	...
	try
		fis:= TFileStream.Create(ButtonedEdit1.Text, fmOpenRead);
		try
...
//					c:= TCompressionStream.Create(clMax, mts);
//					try
//						c.CopyFrom(mos[i], mos[i].Size);
//						finally
//						c.Free;
//						end;
