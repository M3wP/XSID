...
//	URangeEncoder.RangeEncoder:= TRangeEncoder.Create;
	...
	try
		...
		if  fh.system = 2 then
			AXSIDConfig.System:= cstNTSC
		else
			AXSIDConfig.System:= cstPAL;
		AXSIDConfig.UpdateRate:= TC64UpdateRate(fh.updateRate);
		...
		if  sd.sidType = 2 then
			AXSIDConfig.Model:= csmMOS8580
		else
			AXSIDConfig.Model:= csmMOS6581;
		AXSIDConfig.SIDParams.Text:= string(s);
			...
			if  fs.Read(md, SizeOf(TXSIDMetaDataRec)) = SizeOf(TXSIDMetaDataRec) then
				if  md.tag = 'XSMD' then
					if  md.size > (SizeOf(TXSIDMetaDataRec) - 8) then
						...
						AXSIDConfig.MetaData.Text:= string(s);
						end;

			if Assigned(ACallback) then
				ACallback(rfsInitialise, -1, sz);

			AXSIDConfig.ParseSIDParams;
			AXSIDConfig.ParseMetaData;
